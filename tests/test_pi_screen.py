...
class PiScreenHelperTests(unittest.TestCase):
    ...
    def test_visible_pending_items_limits_homepage_queue_to_four(self):
        items = [{"pendingId": str(index)} for index in range(7)]

        visible = pi_screen.visible_pending_items(items)

        self.assertEqual([item["pendingId"] for item in visible], ["0", "1", "2", "3"])
    ...
    def test_touch_fonts_are_sized_for_small_screen_finger_targets(self):
        ...
        self.assertEqual(pi_screen.MAX_HOME_PENDING, 4)
        self.assertEqual(pi_screen.QUEUE_GRID_COLUMNS, 2)
